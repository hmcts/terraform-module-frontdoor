...
resource "azurerm_frontdoor_custom_https_configuration" "https" {
  ...
  custom_https_configuration {
    ...
    azure_key_vault_certificate_secret_name    = lookup(data.azurerm_key_vault_secret.certificate, element(split("/", each.value), 10), null).name
    azure_key_vault_certificate_secret_version = lookup(data.azurerm_key_vault_secret.certificate, element(split("/", each.value), 10), null).version
