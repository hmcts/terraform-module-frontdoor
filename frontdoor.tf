...
resource "azurerm_cdn_frontdoor_route" "routing_rule_A" {
  ...
  dynamic "cache" {
    ...
    content {
      compression_enabled           = lookup(each.value, "compression_enabled", false)
      query_string_caching_behavior = lookup(each.value, "query_string_caching_behavior", "")
      query_strings                 = lookup(each.value, "query_strings", [])
      content_types_to_compress     = lookup(each.value, "content_types_to_compress", [])
    }
  }
}
